 ...
 */
// vertical.config is optional — not all projects use the template config
let _verticalConfig: { aiSystemPrompt?: string } | null = null
try {
  // eslint-disable-next-line @typescript-eslint/no-require-imports
  _verticalConfig = require('@/vertical.config').default
} catch {
  // project doesn't have vertical.config — use built-in default
}
...
export async function aiChat(
  ...
  const system = systemPrompt ?? _verticalConfig?.aiSystemPrompt ?? _aiSystemPrompt
